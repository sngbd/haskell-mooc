...
firstHalf s = if mod (length (s)) 2 == 0 then take (div (length (s)) 2) s
              else take (div ((length (s)) + 1) 2) s
...
capitalizeFirst x = [toUpper (x !! 0)] ++ drop 1 x
...
joinToLength len li = [first ++ second | first <- li, second <- li, length (first ++ second) == len]
