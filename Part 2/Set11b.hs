...
appendAll r xs = do
    readref <- readIORef r
    writeIORef r (readref ++ intercalate "" xs)
...
hSelectLines h xs = do
    ls <- hFetchLines h
    selectHelper h xs ls

selectHelper h [] ls = return []
selectHelper h (x:xs) ls = do
    next <- (selectHelper h xs ls)
    return ([ls !! (x - 1)] ++ next)
