...
prettyPrint size coord = iter 1 1 size coord

iter x y size coord
    | y == (size + 1) && x == size = "\n"
    | y == (size + 1)              = "\n" ++ iter (x + 1) 1 size coord
    | elem (x,y) coord            = "Q" ++ iter x (y + 1) size coord
    | otherwise                    = "." ++ iter x (y + 1) size coord
...
danger candidate (x:xs) = (sameRow candidate x 
                          || sameCol candidate x
                          || sameDiag candidate x
                          || sameAntidiag candidate x)
                          || danger candidate xs
...
prettyPrint2 size stack = iter2 1 1 size stack

iter2 x y size coord
    | y == (size + 1) && x == size = "\n"
    | y == (size + 1)              = "\n" ++ iter2 (x + 1) 1 size coord
    | elem (x,y) coord             = "Q" ++ iter2 x (y + 1) size coord
    | danger (x,y) coord           = "#" ++ iter2 x (y + 1) size coord
    | otherwise = "." ++ iter2 x (y + 1) size coord
--------------------------------------------------------------------------------
...
fixFirst n (x:xs)
    | fix n (x:xs) == [] = Nothing
    | otherwise = Just $ fix n (x:xs) 
    where 
        fix n [] = []
        fix n (x:xs) = if (fst x <= n && snd x <= n) then 
                            if danger x (xs) then fix n (nextCol x:xs) 
                            else x:fix n xs 
                       else []
...
step n stack
    | isNothing $ fixFirst n stack = backtrack stack
    | otherwise = continue (fromJust (fixFirst n stack))
