...
split s = if length (words s) == 2 then Just ((words s) !! 0, (words s) !! 1) 
          else Nothing
...
checkNumber (s, t) = if isNothing (check (s ++ t)) then Nothing 
                     else Just (s, t)
  where 
    check [] = Just "no numbers"
    check (x:xs)
      | isNumber x = Nothing
      | otherwise = check xs
...
checkCapitals (for,sur) = if isUpper (head for) && isUpper (head sur) then Just (for, sur)
                          else Nothing
...
winner scores player1 player2 = look ?> check
  where 
    look = case isNothing (lookup player1 scores) 
                || 
                isNothing (lookup player2 scores) 
                of True -> Nothing
                   _    -> max (lookup player1 scores) 
                               (lookup player2 scores)
    check s
      | lookup player1 scores == Just s = Just player1
...
selectSum [] [] = Just 0
selectSum xs is
  | (length xs - 1) < (foldr max 0 is) || 0 > (foldr min 0 is) = Nothing
  | otherwise = Just (sum (safeIndex xs is))

safeIndex xs [] = []
safeIndex xs (i:is) = (xs !! i):safeIndex xs is
...
countAndLog f xs = Logger (fmap show (filter f xs)) (length (fmap show (filter f xs)))
...
balance accountName = BankOp (fBank)
  where fBank b = (Map.findWithDefault 0 accountName (extract b), b)
        extract (Bank ex) = ex
...
count x = do
  current <- get
  if isNothing(lookup x current) 
    then put ((x,1):current)
  else 
    put ((x,(fromJust (lookup x current) + 1)):filter ((x /= ). fst) current)
