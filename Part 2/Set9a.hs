...
countValid s = length $ filter (== True) $ map check s
  where check s = if (length s >= 5) && (s !! 2 == s !! 4) || (s !! 3 == s !! 5) then True else False
...
sumSuccess s = if (countLeft s /= length s) then Right (sumSuccessHelper s) else Left "no data"

sumSuccessHelper [] = 0
sumSuccessHelper ((Right x):xs) = x + sumSuccessHelper xs
sumSuccessHelper ((Left x):xs) = sumSuccessHelper xs

countLeft [] = 0
countLeft ((Left x):xs) = 1 + countLeft xs
countLeft ((Right x):xs) = countLeft xs
...
data Lock = Locked Bool String
  deriving Show

-- aLock should be a locked lock with the code "1234"
aLock :: Lock
aLock = Locked True "1234"

-- isOpen returns True if the lock is open
isOpen :: Lock -> Bool
isOpen (Locked False code) = True
isOpen (Locked True code) = False

-- open tries to open the lock with the given code. If the code is
-- wrong, nothing happens.
open :: String -> Lock -> Lock
open trycode (Locked True code) = case (trycode == code) of True -> Locked False code
                                                            False -> Locked True code
open trycode (Locked False code) = Locked False code

-- lock closes a lock. If the lock is already closed, nothing happens.
lock :: Lock -> Lock
lock (Locked False code) = Locked True code
lock (Locked True code) = Locked True code

-- changeCode changes the code of an open lock. If the lock is closed,
-- nothing happens.
changeCode :: String -> Lock -> Lock
changeCode newcode (Locked False code) = Locked False newcode
changeCode newcode (Locked True code) = Locked True code
...
compose [] ys = []
compose (x:xs) ys = elim (fst x, lookup (snd x) ys)
  where elim (x, Just y) = (x, y):compose xs ys
        elim (x, Nothing) = compose xs ys
...
permute ind [] = []
permute ind x = permuteHelper (length x) 0 ind x

permuteHelper base i ind x
  | i == base = []
  | otherwise = (x !! (fromJust $ elemIndex i ind)):permuteHelper base (i + 1) ind x