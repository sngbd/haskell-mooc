...
greetText s = T.pack "Hello, " <> T.take 15 s <> 
            case T.length s > 15 of True -> T.pack "...!"
                                    False -> T.pack "!"
...
shout :: T.Text -> T.Text
shout s = helper (T.words s)
    where
        helper [] = T.pack ""
        helper [x] = T.toUpper x
        helper (x:y:xs) = T.toUpper x <> T.pack " " <> y <> 
                        if xs /= [] then T.pack " " <> helper xs 
                        else helper xs
...
longestRepeat s = if (s == T.pack "") then 0 
                  else maximum $ map length $ words $ helper (T.unpack s)
    where
        helper [] = []
        helper [x] = x:[]
        helper (x:xs) = if (x == head xs) then x:helper xs else x:[] ++ " " ++ helper xs
...
byteRange b = if (b == B.pack []) then 0 else maximum (B.unpack b) - minimum (B.unpack b)
...
pingpong b = BL.cycle $ BL.append (BL.fromStrict b) (BL.reverse (BL.fromStrict b))
