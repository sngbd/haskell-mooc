...
mapM2 op (x:xs) (y:ys) = fmap (:) (op x y) <*> mapM2 op xs ys
mapM2 op _ _ = do return []
...
visit maze place = do
  current <- get 
  if elem place current 
    then do return ()
  else do 
    modify (place:) >> mapM_ (visit maze) beside
  where
    (Just beside) = lookup place maze
...
path maze place1 place2 = elem place2 beside
  where (_, beside) = runState (visit maze place1) []
...
f1 k acc x = if (acc+x) > k then Nothing else Just (x+acc)
