...
fmap2 f n = (fmap . fmap) f n

fmap3 :: (Functor f, Functor g, Functor h) => (a -> b) -> f (g (h a)) -> f (g (h b))
fmap3 f n = (fmap . fmap . fmap) f n
...
count e f = length (filter (== e) (toList f))
...
instance Foldable List where
  foldr f i Empty = i
  foldr f i (LNode a x) = (f a (foldr f i x))
...
instance Foldable TwoList where
  ...
  foldr f i (TwoNode a b x) = (f a (f b (foldr f i x)))
...
instance Functor Tree where
  fmap fx Leaf = Leaf
  fmap fx (Node a Leaf Leaf) = Node (fx a) Leaf Leaf
  fmap fx (Node a Leaf r) = Node (fx a) Leaf (fmap fx r)
  fmap fx (Node a l Leaf) = Node (fx a) (fmap fx l) Leaf
  fmap fx (Node a l r) = Node (fx a) (fmap fx l) (fmap fx r)

sumTree :: Monoid m => Tree m -> m
sumTree Leaf = mempty
sumTree (Node a Leaf Leaf) = a
sumTree (Node a Leaf r) = a <> (sumTree r)
sumTree (Node a l Leaf) = (sumTree l) <> a
