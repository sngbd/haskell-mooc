...
countHelper (Just x) count = count
...
largestHelper x result = if filter (x <=) result == [] then x:[]
                         else if filter (x ==) result == result then x:result
                         else result
