...
distinct x = if nub x == x then True else False
...
middle x y z = if (min (max x y) z) == x || (max (min x y) z) == x then x
               else if (min (max y x) z) == y || (max (min y x) z) == y then y
               else z
-- bad code
...
incrementKey k [] = []
incrementKey k (v:vs) = if fst v == k then (k, snd v + 1):incrementKey k vs 
                        else v:incrementKey k vs
...
transfer from to amount bank = if (Map.member from bank) && (Map.member to bank) 
                               then if (Map.findWithDefault 0 from bank >= amount) && (amount >= 0)
                                    then Map.adjust ((+) amount) (to) (Map.adjust (- amount +) (from) (bank)) else bank
                               else bank
...
maxIndex arr = head $ map fst $ filter ((==maximum (elems arr)) . snd) (assocs arr)
