...
buyOneMore (MkShoppingEntry s x y) = (MkShoppingEntry s x (y + 1))
...
setName name (Person x y) = Person x name

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age (Person x y) = Person age y
...
tick :: UpDown -> UpDown
tick (Increase x) = (Increase $ x + 1)
tick (Decrease x) = (Decrease $ x - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (Increase x) = (Decrease x)
toggle (Decrease x) = (Increase x)
...
rgb Green = [0,1,0]
rgb Blue = [0,0,1]
rgb (Mix color1 color2) = map (/ 2.0) (zipWith (+) (rgb color1) (rgb color2))
...
toNatHelper n
    | n == 0 = Zero
    | otherwise = PlusOne $ toNatHelper (n-1)
...
fromBin :: Bin -> Int
fromBin End = 0
fromBin (O b) = 0 + fromBin b * 2
fromBin (I b) = 1 + fromBin b * 2

toBin :: Int -> Bin
toBin 0 = O End
toBin 1 = I End
toBin n = if mod n 2 == 0 then inc (I $ toBin (div n 2))
          else inc (O $ toBin (div n 2))
