...
data State = Start | Error | Finished | AfterEggs | AfterFlour | AfterSugar | AfterEither | AfterMix
  deriving (Eq,Show)

step :: State -> Event -> State
step Start AddEggs = AfterEggs
step AfterEggs AddFlour = AfterFlour
step AfterFlour AddSugar = AfterEither
step AfterEggs AddSugar = AfterSugar
step AfterSugar AddFlour = AfterEither
step AfterEither Mix = AfterMix
...
average (x :| []) = x
average (x :| xs) = foldr (+) 0 (x:xs) / fromIntegral ((length xs) + 1)
...
reverseNonEmpty (x :| []) = (x :| [])
reverseNonEmpty (x :| xs) = (last (xs) :| reverse (x:(take (length (xs) - 1) xs)))
...
passwordAllowed pass (ContainsSome (x:xs))
  | isInfixOf [x] pass = True 
  | otherwise = passwordAllowed pass (ContainsSome xs)
passwordAllowed pass (DoesNotContain x) = 
  not (passwordAllowed pass (ContainsSome x))
passwordAllowed pass (And x y)
  | (passwordAllowed pass x) && (passwordAllowed pass y) = True
  | otherwise = False
passwordAllowed pass (Or x y)
  | (passwordAllowed pass x) || (passwordAllowed pass y) = True
  | otherwise = False
...
data Arithmetic = ArIntStr Integer String
  deriving Show

literal :: Integer -> Arithmetic
literal n = ArIntStr n (show n)

operation :: String -> Arithmetic -> Arithmetic -> Arithmetic
operation op (ArIntStr x a) (ArIntStr y b)
  | op == "+" = (ArIntStr (x + y) (printf "(%s+%s)" a b))
  | op == "*" = (ArIntStr (x * y) (printf "(%s*%s)" a b))
  | op == "-" = (ArIntStr (x - y) (printf "(%s-%s)" a b))
  | otherwise = (ArIntStr (x `div` y) (printf "(%s/%s)" a b))

evaluate :: Arithmetic -> Integer
evaluate (ArIntStr i s) = i

render :: Arithmetic -> String
render (ArIntStr i s) = s
