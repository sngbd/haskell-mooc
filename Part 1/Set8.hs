...
blendColor (Color a b c) (Color x y z) = (Color (div (a + x) 2) 
                                                (div (b + y) 2) 
                                                (div (c + z) 2))

combine :: (Color -> Color -> Color) -> Picture -> Picture -> Picture
combine f (Picture a) (Picture b) = Picture combined
  where combined (Coord x y) = f (a (Coord x y)) (b (Coord x y))
...
union (Shape f) (Shape g) = Shape h
  where h (Coord x y) = (f (Coord x y)) || (g (Coord x y))

cut :: Shape -> Shape -> Shape
cut (Shape f) (Shape g) = Shape h
  where h (Coord x y) = (f (Coord x y)) && (not ((f (Coord x y)) && (g (Coord x y))))
------------------------------------------------------------------------------
...
paintSolid color (Shape f) (Picture base) = Picture overwrite
  where overwrite (Coord x y) = if (f (Coord x y)) then color else base (Coord x y)
...
paint (Picture pat) (Shape f) (Picture base) = Picture overwrite
  where overwrite (Coord x y) = if (f (Coord x y)) then pat (Coord x y) else base (Coord x y)
------------------------------------------------------------------------------
...
instance Transform Fill where
  apply (Fill color) picture = Picture filled
    where filled (Coord x y) = color

data Zoom = Zoom Int
  deriving Show

instance Transform Zoom where
  apply (Zoom z) (Picture f) = Picture zoomed
    where zoomed (Coord x y) = f (Coord (div x z) (div y z))

data Flip = FlipX | FlipY | FlipXY
  deriving Show

instance Transform Flip where
  apply FlipX (Picture f) = Picture flipped
    where flipped (Coord x y) = f (Coord (-x) y)
  apply FlipY (Picture f) = Picture flipped
    where flipped (Coord x y) = f (Coord x (-y))
  apply FlipXY (Picture f) = Picture flipped
    where flipped (Coord x y) = f (Coord y x)
------------------------------------------------------------------------------
...
instance Transform Blur where
  apply Blur (Picture f) = Picture blurred
    where blurred (Coord x y) = (f (Coord x y) 
                              +> f (Coord (x+1) y) 
                              +> f (Coord (x-1) y) 
                              +> f (Coord x (y+1)) 
                              +> f (Coord x (y-1))) 
...
(/>) :: Color -> Int -> Color
(Color r g b) /> n = (Color (div r n) (div g n) (div b n))
