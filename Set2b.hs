...
oddFactorial 1 = 1
oddFactorial x = if mod x 2 == 0 then oddFactorial (x - 1)
                 else x * oddFactorial (x - 1)
...
leftpad str n = if length str < n then leftpad (" " ++ str) n
                else str
...
smallestDivisorHelper n k = if (mod n k) == 0 then k
...
isPrime n = if (n == 0 || n == 1) then False
